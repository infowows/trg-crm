  ...
  try {
    ...
    if (!servicePackage) {
      return NextResponse.json(
        { success: false, message: "Không tìm thấy gói dịch vụ" },
        { status: 404 },
      );
    }
    ...
    return NextResponse.json(
      { success: false, message: "Không thể tải thông tin gói dịch vụ" },
      { status: 500 },
    );
  }
}
...
export async function PUT(
  ...
  try {
    ...
    const updatedPackage = await ServicePackage.findByIdAndUpdate(id, body, {
      new: true,
      runValidators: true,
    });
