...
const CreateQuotation = () => {
        ...
        return quotationPackages.reduce((total, qp) => {
            return (
                ...
                qp.packages.reduce((pkgTotal, pkg) => {
                    return (
                        pkgTotal + (pkg.isSelected ? pkg.totalPrice || 0 : 0)
                    );
                }, 0)
            );
        }, 0);
    ...
    const calculatePackagePrices = () => {
        ...
        setSelectedPackages((prev) =>
            prev.map((pkg) => {
                const totalPrice = pkg.isSelected
                    ? pkg.unitPrice * totalVolume
                    : 0;
                console.log(
                ...
                try {
                    ...
                    if (pricingData.success && pricingData.data.length > 0) {
                        ...
                        pricingData.data.forEach((pricing: any) => {
                            ...
                            const existing =
                                latestPricingByPackage.get(packageName);
    ...
    const addQuotationPackage = () => {
        if (
            ...
            toast.error(
                "Vui lòng chọn nhóm dịch vụ, dịch vụ và ít nhất một gói",
            );
            return;
        }
