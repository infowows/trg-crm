...
const EditOpportunity = ({ params }: { params: Promise<{ id: string }> }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [custRes, servRes, oppRes] = await Promise.all([
        ...
        if (oppRes.ok) {
          ...
          setFormData({
            customerRef: opp.customerRef?._id || opp.customerRef || "",
            demands: opp.demands || [],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(err.message || "Không thể cập nhật cơ hội");
      }
    } catch (err: any) {
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            ...
            >
              ...
              {loading ? "Đang xử lý..." : "Cập nhật cơ hội"}
            </button>
          </div>
        </div>
      </form>
