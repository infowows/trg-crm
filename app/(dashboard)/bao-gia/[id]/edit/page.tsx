  ...
  const loadQuotation = async () => {
    try {
      ...
      if (data.success) {
        ...
        if (quotationData.customerRef && customers.length > 0) {
          const customer = customers.find(
            (c) => c._id === quotationData.customerRef,
          );
          if (customer) {
        ...
        if (quotationData.surveyRef && surveys.length > 0) {
          const survey = surveys.find((s) => s._id === quotationData.surveyRef);
          if (survey) {
